  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    if (!wallet.publicKey || !wallet.signTransaction) {
      toast(
        customToast(
          ...
          <BiWallet size={24} className="text-sky-500" />
        )
      );
      return;
    }
    ...
    if (!imageFile) {
      toast(
        customToast(
          "Please select image.",
          <FaRegImage size={24} className="text-pink-500" />
        )
      );
      return;
    }
    ...
    try {
      ...
      if (walletBalance < lamports) {
        toast(
          customToast(
            ...
            <TbCurrencySolana size={24} className="text-red-500" />
          )
        );
        return;
      }
      ...
      toast(
        customToast(
          "Token Created",
          <GoCheckCircleFill size={24} className="text-green-500" />
        )
      );
      setShowToken(true);
      ...
      toast(
        customToast(
          ...
          <BiSolidError size={24} className="text-red-500" />
        )
      );
    } finally {
      setIsLoading(false);
